...
const HardwareStore: React.FC = () => {
  ...
  const generateDnaString = () => {
    ...
    const header = `--- HARDWARE BUILD DNA: ${selectedProduct.name.toUpperCase()} ---`;
    const body = Object.entries(dnaData)
      .filter(([key]) => !['operator_name', 'operator_email', 'operator_phone'].includes(key))
  ...
  const handleSubmit = async () => {
    if (!dnaData.operator_name || !dnaData.operator_email || !dnaData.operator_phone || !dnaData.payment_identifier) {
      alert("Incomplete Transmission Parameters: Name, Phone, Email, and Payment ID are mandatory.");
      return;
    }
    ...
    const payload = {
      ...
      phone: dnaData.operator_phone,
      service: `Hardware_Order: ${selectedProduct?.name}`,
      payment: `${dnaData.payment_method}: ${dnaData.payment_identifier}`,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          ...
          <div className="hidden md:block text-[10px] font-mono text-gray-700 uppercase tracking-widest text-right">
            ...
            Engineering_Protocol: V4.1_STABLE
          </div>
        </div>
                  ...
                  <div className="space-y-10 animate-fadeIn">
                    <div>
                       <div className="flex items-center gap-2 mb-2">
                         ...
                         <span className="text-[10px] uppercase font-black tracking-[0.4em] text-[#E5B1B1]">Configuring Hardware: {selectedProduct.name}</span>
                       </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-2">
                          ...
                          <input 
                            ...
                            value={dnaData.operator_phone || ''}
                            onChange={(e) => setDnaData({ ...dnaData, operator_phone: e.target.value })}
                          />
                        </div>
                    ...
                    <div className="space-y-6 pt-4 border-t border-white/5">
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          <label className="text-[9px] uppercase tracking-[0.3em] text-[#E5B1B1] font-black">Billing Platform</label>
                          <select 
                            ...
                            onChange={(e) => setDnaData({ ...dnaData, payment_method: e.target.value })}
                            value={dnaData.payment_method || 'invoice'}
                          >
                        ...
                        <div className="space-y-2">
                          <label className="text-[9px] uppercase tracking-[0.3em] text-[#E5B1B1] font-black">Platform Identifier</label>
                          <input 
                            className="w-full bg-black/50 border border-white/10 p-4 text-white font-mono text-sm focus:border-[#E5B1B1]/50 outline-none"
                            placeholder={dnaData.payment_method === 'invoice' || dnaData.payment_method === 'paypal' ? 'BILLING_EMAIL' : 'USER_HANDLE'}
                    ...
                    <div className="space-y-8 pt-4 border-t border-white/5">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] uppercase font-black tracking-[0.4em] text-gray-500">03 / Configuration Logic</span>
                      </div>
                      <div className="font-serif italic text-lg text-gray-300 leading-[2.5]">
                        {selectedProduct.id === 'foundational-node' && (
                          <div>
                            ...
                            The primary contact identity encoded should be 
                            <input 
                              className="bg-transparent border-b border-[#E5B1B1]/50 outline-none text-[#E5B1B1] px-2 mx-1 placeholder:text-gray-700 min-w-[150px]" 
                  ...
                  <div className="py-20 text-center animate-fadeIn space-y-10">
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-gray-400 font-mono text-xs uppercase tracking-widest max-w-sm mx-auto leading-relaxed">
                        Project DNA for {selectedProduct.name} has been transmitted. 
                      </p>
                    </div>
                    
                    <div className="bg-black/40 border border-[#E5B1B1]/30 p-8 max-w-md mx-auto">
                       <p className="text-[#E5B1B1] text-xs italic mb-6">
                         "Build parameters logged. I will reach out to {dnaData.operator_email} with a formal invoice and hardware deployment schedule."
                       </p>
                       <button 
                        onClick={closeModal}
